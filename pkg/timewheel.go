...
func CreateTimeWheel(interval time.Duration, slotNums int, job jobs.JobCommand) *TimeWheel {
	once.Do(func() {
...
func New(interval time.Duration, slotNums int, job jobs.JobCommand) *TimeWheel {
...
func (tw *TimeWheel) AddTask(interval time.Duration, taskName TaskName, createdTime time.Time, jobs jobs.JobCommand) error {
...
func (tw *TimeWheel) checkAndRunTask() {
	...
	if currentList != nil {
		for item := currentList.Front(); item != nil; {
			...
			// 执行任务时，Task.job是第一优先级，然后是TimeWheel.jobs
			if task.job != nil {
				go task.job.Execute(context.Background())
			} else if tw.job != nil {
				go tw.job.Execute(context.Background())
			} else {
				fmt.Println(fmt.Sprintf("The task %v don't have jobs to run", task.TaskName))
			}
